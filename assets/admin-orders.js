(function () {
  ...
  function renderSummaryTab(order) {
    ...
    return '<section class="cs-detail-card"><h3>Operasyon Özeti</h3><div class="cs-grid-3">' +
      ...
      '</div><div class="cs-action-row"><button class="cs-btn" type="button" data-order-action="mark_preparing">Hazırlanıyor işaretle</button>' + (order.payment_method === 'bank_transfer' || order.payment_status === 'awaiting_transfer' || order.status === 'pending_bank_transfer' ? '<button class="cs-btn" type="button" data-resend-email="bank_transfer_reminder">Ödeme bekleniyor maili gönder</button><button class="cs-btn cs-btn-danger" type="button" data-order-action="mark_bank_transfer_not_received">Ödeme alınamadı ve iptal et</button>' : '') + '<button class="cs-btn cs-btn-dark" type="button" data-detail-tab="shipment">Kargo ekle/güncelle</button><button class="cs-btn" type="button" data-detail-tab="emails">E-posta geçmişi</button><button class="cs-btn" type="button" data-detail-tab="invoice">Fatura kaydı</button><button class="cs-btn" type="button" data-detail-tab="returns">İade talebi</button></div></section>' +
  ...
  function renderEmailsTab(order) {
    ...
    var buttons = [['order_created', 'Sipariş onayını tekrar gönder'], ['payment_success', 'Ödeme onayını tekrar gönder'], ['payment_confirmed_manual', 'Manuel ödeme onayını gönder'], ['order_preparing', 'Hazırlanıyor e-postasını gönder'], ['shipment_created', 'Kargo e-postasını tekrar gönder'], ['shipment_delivered', 'Teslimat e-postasını tekrar gönder']];
    if (order.payment_method === 'bank_transfer' || order.payment_status === 'awaiting_transfer' || order.status === 'pending_bank_transfer') buttons.splice(1, 0, ['bank_transfer_pending', 'Havale/EFT bekleme e-postasını gönder'], ['bank_transfer_reminder', 'Ödeme bekleniyor hatırlatma maili gönder'], ['bank_transfer_not_received_cancelled', 'Ödeme alınamadı / iptal maili gönder']);
  ...
  function emailLabel(type) {
    var map = { order_created: 'Sipariş onayı', bank_transfer_pending: 'Havale/EFT bekleniyor', bank_transfer_reminder: 'Havale/EFT hatırlatma', bank_transfer_not_received_cancelled: 'Havale/EFT alınamadı / iptal', payment_success: 'Ödeme onayı', payment_confirmed_manual: 'Manuel ödeme onayı', payment_failed: 'Ödeme başarısız', order_preparing: 'Hazırlanıyor e-postası', order_packed: 'Paketlendi e-postası', shipment_created: 'Kargo e-postası', shipment_updated: 'Kargo güncellemesi', shipment_delivered: 'Teslimat e-postası', return_request_received: 'İade talebi', return_approved: 'İade onayı', return_rejected: 'İade reddi', refund_completed: 'Refund tamamlandı', review_request: 'Yorum isteği' };
  ...
  function handleOrderAction(action) {
    ...
    if (action === 'mark_bank_transfer_not_received') updateOrder({ id: orderId, action: 'mark_bank_transfer_not_received', message: 'Havale/EFT ödemesi manuel kontrolde alınamadı; sipariş iptal edildi.' }, 'Ödeme alınamadı ve sipariş iptal edilsin mi?', 'Bu işlem stok rezervasyonunu serbest bırakır ve müşteriye “Havale/EFT ödemeniz alınamadığı için siparişiniz iptal edildi” e-postasını göndermeyi dener.');
  }
