(function () {
  ...
  var CATEGORY_ROUTES = [
    ...
    { key: 'eye-care', title: 'Göz Bakımı', subtitle: 'Göz çevresi için destek bakım.', category: 'Göz Bakımı', href: '/collections/care.html?filter=goz-bakimi', icon: 'eye' },
    { key: 'lip-care', title: 'Dudak Bakımı', subtitle: 'Dudak bakım ürünleri.', category: 'Dudak Bakımı', href: '/collections/care.html?filter=dudak-bakimi', icon: 'heart' }
  ];
  ...
  function bottomNav(active) {
    var items = [
      ...
      ['grid', 'Kategoriler', '/categories.html', 'categories'],
      ['sparkle', 'Keşfet', '/explore.html', 'explore'],
  ...
  function productCard(product, opts) {
    ...
    return '<article class="cm-product-card" data-product-slug="' + esc(product.slug) + '">' +
      ...
      '<div class="cm-product-card__body"><small>' + esc(product.brand.toUpperCase()) + '</small><a class="cm-product-title" href="' + esc(product.url) + '" aria-label="' + title + ' ürün detayını aç"><h3>' + esc(product.name) + '</h3></a><b class="cm-card-price">' + formatPrice(product.price) + '</b>' + ratingHtml(product) + '<div class="cm-card-stock-row">' + statusPill(product.slug) + '</div><div class="cm-card-actions"><button class="cm-card-cart" type="button" data-cm-add-cart="' + esc(product.slug) + '" aria-label="' + title + ' sepete ekle">Sepete Ekle</button><button class="cm-card-bag" type="button" data-cm-add-cart="' + esc(product.slug) + '" aria-label="' + title + ' hızlı sepete ekle">' + svg('bag') + '</button></div><a class="cm-stock-notify-card" data-cm-stock-notify data-product-slug="' + esc(product.slug) + '" href="/favorites.html">Stok gelince haber ver ' + svg('chevron') + '</a><span class="cm-stock-line" data-cm-stock-line data-product-slug="' + esc(product.slug) + '" aria-live="polite"></span></div></article>';
  }
  ...
  function homePage() {
    ...
    var hero = getProductBySlug('anua-heartleaf-77-soothing-toner') || getProductBySlug('anua-heartleaf-pore-control-cleansing-oil') || products[0] || {};
  ...
  function listingPage() {
    ...
    return '<div class="cm-mobile-page cm-mobile-listing">' + header({ back: true }) + '<div class="cm-page-inner cm-page-inner--top">' + searchBar() + '<section class="cm-list-head"><div><h1>' + esc(meta.title) + '</h1><p><span data-cm-product-count>0</span> ürün</p></div></section><div class="cm-list-controls"><button type="button" data-cm-open-filter>Filtrele ' + svg('filter') + '</button><button type="button" data-cm-open-sort>Sırala ' + svg('sort') + '</button></div>' + (chips.length ? '<div class="cm-filter-chips">' + chips.map(function (c) { return c.key ? '<button type="button" data-cm-filter-remove="' + esc(c.key) + '">' + esc(c.label) + ' <b aria-hidden="true">×</b></button>' : '<span>' + esc(c.label) + '</span>'; }).join('') + '</div>' : '') + '<div class="cm-product-grid" data-cm-listing-grid></div></div><div class="cm-mobile-filterbar"><button type="button" data-cm-open-filter>' + svg('filter') + ' Filtrele</button><button type="button" data-cm-open-sort>' + svg('sort') + ' Sırala</button></div>' + bottomNav('categories') + '</div>';
  }
  ...
  function accountPage() {
    ...
    if (!profile.name && !profile.email) return '<div class="cm-mobile-page cm-mobile-account">' + header() + '<div class="cm-page-inner cm-page-inner--top"><h1 class="cm-page-title">Hesabım</h1><div class="cm-empty-state"><strong>Hesabınızı görüntülemek için giriş yapın.</strong><span>Siparişlerinizi, adreslerinizi ve favorilerinizi tek yerden yönetin.</span><a class="cm-btn cm-btn--primary" href="/index.html?auth=login&next=/account/profile.html">Giriş Yap</a><a class="cm-btn" href="/index.html?auth=signup&next=/account/profile.html">Üye Ol</a></div></div>' + bottomNav('account') + '</div>';
    ...
    var summary = getStoredAccountSummary();
    var hasReturns = Array.isArray(summary.returns) && summary.returns.length;
    var hasCoupons = Array.isArray(summary.coupons) && summary.coupons.length;
    return '<div class="cm-mobile-page cm-mobile-account">' + header() + '<div class="cm-page-inner cm-page-inner--top"><h1 class="cm-page-title">Hesabım</h1><section class="cm-profile-card"><div class="cm-avatar">' + esc(initials || 'CS') + '</div><div><strong>' + esc(name) + '</strong><span>' + esc(profile.email || 'Hesap bilgilerinizi yönetin') + '</span>' + (profile.phone ? '<span>' + esc(profile.phone) + '</span>' : '') + (summary.loyalty?.label ? '<em class="cm-membership-badge">' + esc(summary.loyalty.label) + '</em>' : '') + '</div><a href="/account/profile.html?tab=security">Profili Düzenle ' + svg('chevron') + '</a></section><div class="cm-quick-actions cm-quick-actions--account"><a href="/account/profile.html?tab=orders">' + svg('cube') + '<span>Siparişlerim</span></a><a href="/account/profile.html?tab=addresses">' + svg('tag') + '<span>Adreslerim</span></a><a href="/favorites.html">' + svg('heart') + '<span>Favorilerim</span></a>' + (hasCoupons ? '<a href="/account/profile.html?tab=coupons">' + svg('tag') + '<span>Kuponlarım</span></a>' : '') + (hasReturns ? '<a href="/account/returns.html">' + svg('truck') + '<span>İade Taleplerim</span></a>' : '') + '<button type="button" data-cm-logout>' + svg('chevron') + '<span>Çıkış</span></button></div><div data-cm-account-orders>' + sectionHead('Son Siparişlerim', '/account/profile.html?tab=orders') + (orders.length ? orders.slice(0, 2).map(orderCard).join('') : '<div class="cm-empty-state cm-empty-state--inline"><strong>Siparişiniz henüz bulunmuyor.</strong><span>Sipariş verdiğinizde durum kartları burada görünür.</span></div>') + '</div><section class="cm-help-card"><strong>Yardıma mı ihtiyacınız var?</strong><span>Destek ekibimizle iletişime geçin.</span><a href="/contact.html">Destek & İletişim ' + svg('chevron') + '</a></section></div>' + bottomNav('account') + '</div>';
  }
