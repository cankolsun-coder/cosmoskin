(function () {
  ...
  var CATEGORY_ROUTES = {
    cleanse: { title: 'Temizleyiciler', subtitle: 'Cildi kurutmadan arındıran seçili ürünler.', href: '/collections/cleanse.html', category: 'Temizleyiciler' },
    ...
    treat: { title: 'Serum & Ampul', subtitle: 'Hedef bakım odaklı yoğun formüller.', href: '/collections/treat.html', category: 'Serum & Ampul' },
    ...
    protect: { title: 'Güneş Koruyucular', subtitle: 'Günlük SPF seçkisi.', href: '/collections/protect.html', category: 'Güneş Koruyucular' },
    masks: { title: 'Maskeler', subtitle: 'Haftalık destek ve hedef bakım ürünleri.', href: '/collections/masks.html', category: 'Maskeler' },
    blemish: { title: 'Akne & Denge', subtitle: 'Sebum, gözenek ve görünüm dengesi odağı.', href: '/collections/blemish.html', concern: 'blemish' }
  };

  var FALLBACK_IMG = '/assets/img/products/anua/anua-heartleaf-77-soothing-toner-card.webp';
  var HERO = {
    toner: '/assets/img/hero/toner.webp',
    serum: '/assets/img/hero/serum.webp',
    cream: '/assets/img/hero/cream.webp',
    miniSerum: '/assets/img/hero/mini-serum.webp'
  };
  ...
  function normalizeProduct(raw) {
    ...
    return {
      ...
      image: String(raw.image || raw.img || FALLBACK_IMG).trim() || FALLBACK_IMG,
  ...
  function cartTotals() {
    ...
    var shipping = subtotal > 0 && subtotal < 750 ? 59 : 0;
  ...
  function header(options) {
    options = options || {};
    var promo = options.promo === false ? '' : '<div class="cm-promo">' + svg('bag') + '<span>750 TL ve üzeri alışverişlerde ücretsiz kargo!</span></div>';
    var left = options.back ? '<button class="cm-icon-btn" type="button" data-cm-back aria-label="Geri dön">' + svg('back') + '</button>' : '<button class="cm-icon-btn" type="button" data-cm-menu aria-label="Menüyü aç" aria-expanded="false" aria-controls="cm-mobile-menu">' + svg('menu') + '</button>';
    return promo + '<header class="cm-header ' + (options.promo === false ? 'cm-header--bare' : '') + '"><div>' + left + '</div><a href="/index.html" class="cm-wordmark" aria-label="COSMOSKIN anasayfa">COSMOSKIN</a><div class="cm-header__right"><a class="cm-icon-btn" href="/search.html" aria-label="Ara">' + svg('search') + '</a><a class="cm-icon-btn" href="/checkout.html" aria-label="Sepetim">' + svg('bag') + '<span class="cm-badge" data-cm-cart-badge>0</span></a></div></header>';
  }
  ...
  function productCard(product, options) {
    options = options || {};
    var label = product.label || (options.index === 0 ? 'Editör Seçimi' : '');
    var rating = product.ratingText || 'Henüz değerlendirme yok';
  ...
  function brandStrip() {
    var brands = ['anua', 'beauty-of-joseon', 'cosrx', 'round-lab', 'skin1004', 'torriden', 'thank-you-farmer'];
    return '<div class="cm-brand-strip" aria-label="Sevdiğin markalar">' + brands.map(function (b) { return '<img src="/assets/img/brands/' + b + '.svg" alt="' + escapeHtml(b.replace(/-/g, ' ')) + '" loading="lazy">'; }).join('') + '</div>';
  }

  function trustStrip() {
    return '<section class="cm-trust-strip" aria-label="Güven unsurları"><div class="cm-trust-item">' + svg('leaf') + '<strong>%100 Orijinal<br>Ürün</strong></div><div class="cm-trust-item">' + svg('shield') + '<strong>Güvenli<br>Alışveriş</strong></div><div class="cm-trust-item">' + svg('truck') + '<strong>Hızlı<br>Teslimat</strong></div><div class="cm-trust-item">' + svg('cube') + '<strong>Kolay<br>İade</strong></div></section>';
  }

  function selectProducts(predicate, limit) {
    var products = getProducts().filter(predicate || function () { return true; });
    return products.slice(0, limit || products.length);
  }

  function homePage() {
    var products = getProducts();
    var best = products.filter(function (p) { return ['Anua', 'Beauty of Joseon', 'COSRX', 'SKIN1004', 'Torriden'].indexOf(p.brand) !== -1; }).slice(0, 4);
    if (best.length < 4) best = products.slice(0, 4);
    var cleanser = products.find(function (p) { return p.category === 'Temizleyiciler'; }) || best[0];
    var toner = products.find(function (p) { return p.category === 'Tonik & Essence'; }) || best[0];
    var serum = products.find(function (p) { return p.category === 'Serum & Ampul'; }) || best[0];
    var cream = products.find(function (p) { return p.category === 'Nemlendiriciler'; }) || best[0];
    return '<div class="cm-mobile-page cm-mobile-home">' + header({ promo: true }) +
      '<section class="cm-hero" aria-labelledby="cmHomeHeroTitle"><div class="cm-hero__copy"><h1 id="cmHomeHeroTitle">Cildin.<br>Işıltın.<br>Senin hikayen.</h1><p>Seçilmiş Kore cilt bakım ürünleriyle sade, etkili ve güvenli bir bakım deneyimi.</p><div class="cm-hero__actions"><a class="cm-btn cm-btn--primary" href="/allproducts.html">ALIŞVERİŞE BAŞLA</a><a class="cm-btn" href="/collections/routine.html">RUTİNİNİ KEŞFET</a></div></div><div class="cm-hero__products" aria-hidden="true"><img class="p-serum" src="' + HERO.miniSerum + '" alt=""><img class="p-toner" src="' + HERO.toner + '" alt=""><img class="p-cream" src="' + HERO.cream + '" alt=""></div></section>' +
      trustStrip() + brandStrip() +
      '<div class="cm-page-inner"><div class="cm-section-head"><h2>İhtiyacına göre daha hızlı seçim yap</h2><a href="/allproducts.html">Tümünü Gör</a></div><div class="cm-category-row">' +
      categoryCard((cleanser && cleanser.image) || '/assets/img/cleanse.jpg', 'Temizleyiciler', 'Cildi arındır', '/collections/cleanse.html') +
      categoryCard((toner && toner.image) || '/assets/img/hydrate.jpg', 'Tonik & Essence', 'Dengele & hazırla', '/collections/hydrate.html') +
      categoryCard((serum && serum.image) || '/assets/img/treat.jpg', 'Serum & Ampul', 'Hedef bakım', '/collections/treat.html') +
      categoryCard((cream && cream.image) || '/assets/img/care.jpg', 'Nemlendiriciler', 'Nemlendir & koru', '/collections/care.html') +
      '</div><div class="cm-section-head"><h2>Çok Satanlar</h2><a href="/allproducts.html">Tümünü Gör</a></div><div class="cm-product-grid">' + best.map(productCard).join('') + '</div></div>' + bottomNav('home') + '</div>';
  }
  ...
  function routeListingMeta() {
    ...
    if (/\/brands\//.test(path)) {
      ...
      var product = getProducts().find(function (p) { return p.brandSlug === brandSlug || slugify(p.brand) === brandSlug; });
      var brand = product ? product.brand : brandSlug.split('-').map(function (w) { return w.toUpperCase() === 'COSRX' ? 'COSRX' : w.charAt(0).toUpperCase() + w.slice(1); }).join(' ');
      return { title: brand, subtitle: 'Seçili marka ürünleri', category: '', brand: brand };
    }
    var collection = path.match(/\/collections\/([^.?#/]+)\.html/);
    if (collection) {
      var key = collection[1];
      var meta = CATEGORY_ROUTES[key] || { title: 'Seçili Ürünler', subtitle: 'COSMOSKIN seçkisi.', category: '' };
      return meta;
    }
  ...
  function filteredProducts() {
    ...
    if (meta.concern) products = products.filter(function (p) { return (p.concernSlugs || []).indexOf(meta.concern) !== -1 || String(p.keywords).toLocaleLowerCase('tr-TR').indexOf('akne') !== -1; });
    if (meta.brand) products = products.filter(function (p) { return p.brand === meta.brand || slugify(p.brand) === slugify(meta.brand); });
    if (listingState.category && !meta.category) products = products.filter(function (p) { return p.category === listingState.category; });
  ...
  function listingPage() {
    ...
    var chips = [
      ['Tümü', '', '/allproducts.html'], ['Temizleyici', 'Temizleyiciler', '/collections/cleanse.html'], ['Tonik', 'Tonik & Essence', '/collections/hydrate.html'], ['Serum', 'Serum & Ampul', '/collections/treat.html'], ['Güneş', 'Güneş Koruyucular', '/collections/protect.html'], ['Nem', 'Nemlendiriciler', '/collections/care.html'], ['Maskeler', 'Maskeler', '/collections/masks.html']
    ];
    return '<div class="cm-mobile-page cm-mobile-listing">' + header({ promo: false }) + '<div class="cm-page-inner"><form class="cm-search-box" action="/search.html" role="search">' + svg('search') + '<input name="q" type="search" value="' + qValue + '" placeholder="Ürün, marka veya içerik ara..."><button class="cm-filter-btn" type="button" data-cm-open-filter aria-label="Filtreleri aç">' + svg('filter') + '</button></form><div class="cm-chip-row" aria-label="Hızlı kategoriler">' + chips.map(function (chip) { var active = (!meta.category && !chip[1] && !meta.brand) || meta.category === chip[1]; return '<a class="cm-chip ' + (active ? 'is-active' : '') + '" href="' + chip[2] + '">' + chip[0] + '</a>'; }).join('') + '</div><div class="cm-listing-head"><div><h1>' + escapeHtml(meta.title) + '</h1><span data-cm-product-count>' + products.length + ' ürün</span></div><div class="cm-listing-actions"><button class="cm-sort-btn" type="button" data-cm-open-sort>Sırala ' + svg('chevron') + '</button><button class="cm-sort-btn" type="button" data-cm-open-filter>' + svg('filter') + ' Filtrele</button></div></div><div class="cm-product-grid" data-cm-listing-grid>' + (products.length ? products.map(productCard).join('') : emptyState('Eşleşen ürün bulunamadı.', 'Filtreleri temizleyerek tekrar deneyebilirsin.')) + '</div></div>' + bottomNav('list') + '</div>';
  }
  ...
  function productGuide(product) {
    ...
    if (!raw) return categoryFallbackGuide(product);
    return {
      benefits: Array.isArray(raw.benefits) ? raw.benefits : categoryFallbackGuide(product).benefits,
      usage: raw.howToUse || raw.usage || categoryFallbackGuide(product).usage,
      ingredients: Array.isArray(raw.ingredients) ? raw.ingredients.map(function (i) { return i.name || i; }).slice(0, 5).join(', ') : categoryFallbackGuide(product).ingredients,
      suitability: Array.isArray(raw.idealSkinTypes) ? raw.idealSkinTypes.join(', ') : categoryFallbackGuide(product).suitability,
      lead: raw.lead || '',
  ...
  function routinePick(category, goal, period) {
    var products = getProducts();
    var pool = products.filter(function (p) { return p.category === category; });
    var text = String(goal || '').toLocaleLowerCase('tr-TR');
    if (text) {
      var preferred = pool.find(function (p) { return [p.name, p.brand, p.keywords].join(' ').toLocaleLowerCase('tr-TR').indexOf(text) !== -1; });
      if (preferred) return preferred;
    }
    if (period === 'day' && category === 'Güneş Koruyucular') return pool[0] || products[0];
    return pool[0] || products[0];
  }
  ...
  function currentRoutineProducts() {
    ...
    return cats.map(function (cat) { return routinePick(cat, goal, routineState.period); }).filter(Boolean);
  }
  ...
  function routinePage() {
    ...
    var goals = ['Nem', 'Bariyer', 'Işıltı', 'Akne Karşıtı', 'Gözenek', 'Yaşlanma Karşıtı'];
    var skins = ['Kuru', 'Karma', 'Yağlı', 'Hassas'];
    var score = Math.min(98, 86 + routineState.goals.length * 3 + (routineState.skin === 'Hassas' ? 1 : 0));
    return '<div class="cm-mobile-page cm-mobile-routine">' + header({ promo: false }) + '<section class="cm-routine-title"><div class="cm-routine-title__top"><span>AKILLI RUTİN SEÇİMİ</span><span>' + (routineState.period === 'day' ? 'Gündüz' : 'Gece') + '</span></div><h1>Cildin için en doğru rutini birlikte oluşturalım.</h1><p>Hedeflerini seç, cilt tipini ekle. Sana özel rutin ürünleri gerçek ürün verisiyle güncellensin.</p></section><div class="cm-routine-builder"><section class="cm-step-block"><h2>1. Cilt Hedefini Seç</h2><small>Bir veya birkaç hedef seçebilirsin.</small><div class="cm-select-grid">' + goals.map(function (g) { var selected = routineState.goals.indexOf(g) !== -1; return '<button class="cm-select-chip ' + (selected ? 'is-selected' : '') + '" type="button" data-cm-routine-goal="' + escapeHtml(g) + '" aria-pressed="' + selected + '">' + svg(g === 'Nem' ? 'drop' : g === 'Bariyer' ? 'shield' : 'sparkle') + escapeHtml(g) + '</button>'; }).join('') + '</div></section><section class="cm-step-block"><h2>2. Cilt Tipini Seç</h2><small>Ürün uyumu buna göre dengelenir.</small><div class="cm-select-grid cm-select-grid--skin">' + skins.map(function (s) { var selected = routineState.skin === s; return '<button class="cm-select-chip ' + (selected ? 'is-selected' : '') + '" type="button" data-cm-routine-skin="' + escapeHtml(s) + '" aria-pressed="' + selected + '">' + escapeHtml(s) + '</button>'; }).join('') + '</div></section><section class="cm-step-block"><h2>3. Rutin Zamanı</h2><div class="cm-toggle"><button class="' + (routineState.period === 'day' ? 'is-active' : '') + '" type="button" data-cm-routine-period="day" aria-pressed="' + (routineState.period === 'day') + '">' + svg('sun') + ' Gündüz</button><button class="' + (routineState.period === 'night' ? 'is-active' : '') + '" type="button" data-cm-routine-period="night" aria-pressed="' + (routineState.period === 'night') + '">' + svg('moon') + ' Gece</button></div></section><section class="cm-routine-card"><div class="cm-routine-card__head"><div><strong>' + escapeHtml(routineState.goals.join(' + ')) + ' Rutini</strong><span>' + escapeHtml(routineState.skin) + ' cilt için seçildi.</span></div><div class="cm-match">%' + score + '</div></div><div class="cm-routine-products">' + items.map(function (p, idx) { return '<a class="cm-routine-step" href="' + escapeHtml(p.url) + '"><img src="' + escapeHtml(p.image) + '" alt="' + escapeHtml(p.name) + '"><b>' + ['Temizleyici', 'Tonik', 'Serum', routineState.period === 'day' ? 'SPF' : 'Nem'][idx] + '</b><span>' + escapeHtml(p.brand) + '</span></a>'; }).join('') + '</div><div class="cm-set-price"><div><small>Set avantaj fiyatı</small><s>' + formatPrice(subtotal) + '</s><b>' + formatPrice(setPrice) + '</b></div><span>Sepette ürün bazında uygulanır</span></div><button class="cm-btn cm-btn--primary cm-btn--wide" type="button" data-cm-add-routine>TÜM RUTİNİ SEPETE EKLE</button><div class="cm-secondary-actions"><button class="cm-btn" type="button" data-cm-save-routine>Rutini Kaydet</button><a class="cm-btn" href="#routine-commerce">Rutini Gör</a></div></section></div>' + bottomNav('list') + '</div>';
  }
  ...
  function cartContentHtml() {
    ...
    return '<div class="cm-cart-list">' + cart.map(function (item) { return '<article class="cm-cart-row"><a class="cm-cart-row__img" href="' + escapeHtml(item.url) + '"><img src="' + escapeHtml(item.image) + '" alt="' + escapeHtml(item.name) + '"></a><div><h2>' + escapeHtml(item.name) + '</h2><small>' + escapeHtml(item.brand) + '</small><b>' + formatPrice(item.price * item.qty) + '</b><div class="cm-stepper"><button type="button" data-cm-cart-dec="' + escapeHtml(item.slug) + '" aria-label="Adedi azalt">−</button><span>' + item.qty + '</span><button type="button" data-cm-cart-inc="' + escapeHtml(item.slug) + '" aria-label="Adedi artır">+</button></div></div><button class="cm-trash" type="button" data-cm-cart-remove="' + escapeHtml(item.slug) + '" aria-label="Ürünü sepetten çıkar">' + svg('trash') + '</button></article>'; }).join('') + '</div><form class="cm-coupon" data-cm-coupon-form><label>' + svg('tag') + '<input name="coupon" autocomplete="off" placeholder="Kupon kodu"></label><button type="submit">Uygula</button><span class="cm-coupon-status" data-cm-coupon-status></span></form><section class="cm-summary" aria-label="Sipariş özeti"><h2>Sipariş Özeti</h2><div class="cm-summary-row"><span>Ara Toplam</span><b>' + formatPrice(totals.subtotal) + '</b></div><div class="cm-summary-row"><span>Teslimat</span><b>' + (totals.shipping ? formatPrice(totals.shipping) : 'Ücretsiz') + '</b></div><div class="cm-summary-row is-total"><span>Toplam</span><b>' + formatPrice(totals.total) + '</b></div><div class="cm-campaign-note">' + svg('truck') + (totals.subtotal >= 750 ? ' Ücretsiz kargo avantajı uygulandı.' : ' Ücretsiz kargo için ' + formatPrice(750 - totals.subtotal) + ' daha ekleyebilirsin.') + '</div><a class="cm-btn cm-btn--primary cm-btn--wide" href="/checkout.html#checkoutForm" style="margin-top:13px">Ödemeye Geç ' + svg('chevron') + '</a></section><section class="cm-cart-trust"><div>' + svg('lock') + '256-bit SSL<br>Güvenli Ödeme</div><div>' + svg('shield') + '%100 Orijinal<br>Ürün Garantisi</div><div>' + svg('truck') + '1-3 İş Günü<br>Hızlı Teslimat</div></section>';
  }
  ...
  function menuOverlay() {
    ...
    return '<div class="cm-menu-dim" data-cm-menu-close></div><aside class="cm-menu-panel" id="cm-mobile-menu" aria-label="COSMOSKIN mobil menü" aria-hidden="true"><div class="cm-menu-head"><button class="cm-menu-close" type="button" data-cm-menu-close aria-label="Menüyü kapat">' + svg('close') + '</button><div><div class="cm-menu-logo">COSMOSKIN</div><div class="cm-menu-welcome">' + svg('leaf') + ' Merhaba, güzelliğine iyi bak.</div></div></div><nav class="cm-menu-main" aria-label="Genel navigasyon">' + link('grid', 'Kategoriler', '/allproducts.html') + link('tag', 'Markalar', '/brands/anua.html') + link('star', 'Çok Satanlar', '/index.html#bestsellers') + link('bottle', 'Rutinler', '/collections/routine.html') + link('sparkle', 'COSMOSKIN Edit', '/index.html#brands') + link('shield', 'Destek', '/contact.html') + '</nav><section class="cm-menu-section"><div class="cm-menu-section__head"><h3>Hızlı Kategoriler</h3><a href="/allproducts.html">Tümünü Gör</a></div><div class="cm-menu-card-grid">' + card('bottle', 'Temizleyiciler', '/collections/cleanse.html') + card('drop', 'Tonik & Essence', '/collections/hydrate.html') + card('sparkle', 'Serum & Ampul', '/collections/treat.html') + card('cube', 'Nemlendiriciler', '/collections/care.html') + card('shield', 'Güneş Koruyucular', '/collections/protect.html') + card('leaf', 'Maskeler', '/collections/masks.html') + '</div></section><section class="cm-menu-section"><div class="cm-menu-section__head"><h3>Cilt Hedeflerine Göre Keşif</h3><a href="/collections/routine.html">Tümünü Gör</a></div><div class="cm-menu-card-grid">' + card('drop', 'Nem', '/collections/hydrate.html') + card('shield', 'Bariyer', '/collections/care.html') + card('sparkle', 'Işıltı', '/collections/treat.html') + card('leaf', 'Akne & Denge', '/collections/blemish.html') + card('heart', 'Hassasiyet', '/collections/care.html') + card('grid', 'Gözenek & Sebum', '/collections/blemish.html') + '</div></section><section class="cm-menu-section"><div class="cm-menu-section__head"><h3>Sevdiğin Markalar</h3><a href="/brands/anua.html">Tümünü Gör</a></div><div class="cm-brand-cloud">' + ['anua', 'cosrx', 'beauty-of-joseon', 'round-lab', 'skin1004', 'torriden', 'thank-you-farmer'].map(function (b) { return '<a href="' + (b === 'thank-you-farmer' ? '/collections/thank-you-farmer.html' : '/brands/' + b + '.html') + '"><img src="/assets/img/brands/' + b + '.svg" alt="' + escapeHtml(b) + '"></a>'; }).join('') + '</div></section><nav class="cm-service-links" aria-label="Hesap ve destek">' + link('user', 'Hesabım', '/account/profile.html') + link('cube', 'Siparişlerim', '/account/orders.html') + link('heart', 'Favorilerim', '/account/profile.html#favorites') + link('bag', 'Sepetim', '/checkout.html') + link('shield', 'Yardım & Destek', '/contact.html') + link('truck', 'İade ve Teslimat', '/iade-degisim.html') + '</nav></aside>';
  }
  ...
  function sheetHtml(kind) {
    ...
    var brands = Array.from(new Set(getProducts().map(function (p) { return p.brand; }).filter(Boolean))).slice(0, 12);
  ...
  function renderRoutineMobile() {
    ...
    if (root && getPageType() === 'routine') {
      root.innerHTML = routinePage();
      updateBadges();
    }
  }
  ...
  function renderPdpMobile() {
    var root = document.getElementById(ROOT_ID);
    if (root && getPageType() === 'pdp') {
      root.innerHTML = pdpPage();
  ...
  function fallbackToggleFavorite(button) {
    if (window.initFavoriteButtons && button.dataset.favoriteBound) return;
  ...
  function mount() {
    ...
    if (!root) {
      root = document.createElement('div');
      root.id = ROOT_ID;
      main.insertAdjacentElement('afterbegin', root);
    }
    var html = type === 'pdp' ? pdpPage() : type === 'cart' ? cartPage() : type === 'routine' ? routinePage() : type === 'listing' ? listingPage() : homePage();
    root.innerHTML = html;
    var drawer = document.getElementById(DRAWER_ID);
    if (!drawer) {
      drawer = document.createElement('div');
      drawer.id = DRAWER_ID;
      drawer.innerHTML = menuOverlay();
  ...
  function bindDelegates() {
    ...
    document.addEventListener('click', function (event) {
      ...
      if (add) {
        ...
        addItemsToCart([{ id: product.slug, slug: product.slug, name: product.name, brand: product.brand, price: product.price, image: product.image, url: product.url, qty: qty }]);
        return;
      }
      ...
      var inc = target.closest('[data-cm-cart-inc]');
      if (inc) { updateCartQuantity(inc.getAttribute('data-cm-cart-inc'), 1); return; }
      var dec = target.closest('[data-cm-cart-dec]');
      if (dec) { updateCartQuantity(dec.getAttribute('data-cm-cart-dec'), -1); return; }
      var rem = target.closest('[data-cm-cart-remove]');
      if (rem) { removeCartItem(rem.getAttribute('data-cm-cart-remove')); return; }
      ...
      var sort = target.closest('[data-cm-sort]');
      if (sort) { listingState.sort = sort.getAttribute('data-cm-sort'); closeSheet(); renderListingGrid(); return; }
      var cat = target.closest('[data-cm-filter-category]');
      if (cat) { listingState.category = cat.getAttribute('data-cm-filter-category'); renderListingGrid(); openSheet('filter'); return; }
      var brand = target.closest('[data-cm-filter-brand]');
      if (brand) { listingState.brand = brand.getAttribute('data-cm-filter-brand'); renderListingGrid(); openSheet('filter'); return; }
      var goal = target.closest('[data-cm-routine-goal]');
      if (goal) { var value = goal.getAttribute('data-cm-routine-goal'); var idx = routineState.goals.indexOf(value); if (idx === -1) routineState.goals.push(value); else if (routineState.goals.length > 1) routineState.goals.splice(idx, 1); renderRoutineMobile(); return; }
      var skin = target.closest('[data-cm-routine-skin]');
      if (skin) { routineState.skin = skin.getAttribute('data-cm-routine-skin'); renderRoutineMobile(); return; }
      var period = target.closest('[data-cm-routine-period]');
      if (period) { routineState.period = period.getAttribute('data-cm-routine-period'); renderRoutineMobile(); return; }
      ...
      if (fav && fav.closest('#' + ROOT_ID)) { fallbackToggleFavorite(fav); return; }
    ...
    document.addEventListener('submit', function (event) {
      if (event.target.matches('[data-cm-coupon-form]')) {
        event.preventDefault();
        var status = event.target.querySelector('[data-cm-coupon-status]');
        var input = event.target.querySelector('input[name="coupon"]');
        if (status) status.textContent = input && input.value ? 'Kupon ödeme adımında doğrulanacak.' : 'Kupon kodu girilmedi.';
      }
    });
    document.addEventListener('keydown', function (event) {
      if (event.key === 'Escape') { closeMenu(); closeSheet(); }
    });
  ...
  function loadGuides() {
    ...
    fetch('/assets/data/product-guides.json', { cache: 'default' })
      ...
      .then(function (json) { guides = json || {}; renderPdpMobile(); })
