(function () {
  ...
  function productCard(p, mode) { mode = mode || 'cart'; var cta = mode === 'inspect' ? 'Ürünü İncele' : 'Sepete Ekle'; var action = mode === 'inspect' ? '<a class="cs-mini-btn dark" href="' + escapeHtml(p.url) + '">' + cta + '</a>' : '<button class="cs-mini-btn dark" type="button" data-add-product-cart="' + escapeHtml(p.product_slug) + '">' + cta + '</button>'; return '<article class="cs-product-mini"><a class="cs-product-mini__media" href="' + escapeHtml(p.url) + '"><img src="' + escapeHtml(p.image || '/assets/logo-mark-beige.png') + '" alt="' + escapeHtml(p.product_name) + '" loading="lazy"></a><div><small>' + escapeHtml(p.brand.toUpperCase()) + '</small><strong>' + escapeHtml(p.product_name) + '</strong><p>' + escapeHtml(p.category || 'Tonik & Essence') + '</p><span class="cs-stock-dot">● Stokta</span><div class="cs-product-mini__bottom"><b>' + escapeHtml(formatMoney(p.price)) + '</b>' + action + '<button class="cs-heart-btn" type="button" data-add-favorite="' + escapeHtml(p.product_slug) + '" aria-label="Favoriye ekle">♡</button></div></div></article>'; }
  ...
  function renderAddresses() { var el = $('#addressesPanel'); if (!el) return; var addresses = state.summary?.addresses || []; el.innerHTML = '<div class="cs-tab-title"><div><h1>Adreslerim</h1><p>Teslimat ve fatura adreslerini yönet.</p></div><button class="cs-pill-btn" type="button" id="addAddressBtn">Yeni Adres Ekle</button></div>' + (addresses.length ? '<div class="cs-address-grid">' + addresses.map(function (a) { return '<article class="cs-address-card"><h3>' + escapeHtml(a.title || 'Adres') + '</h3><p>' + escapeHtml([a.address_line, a.district, a.city].filter(Boolean).join(', ')) + '</p><small>' + escapeHtml(a.phone || '') + '</small></article>'; }).join('') + '</div>' : emptyState('Kayıtlı adresin bulunmuyor.', 'Checkout sırasında hızlı kullanım için teslimat adresi ekleyebilirsin.', 'Adres Ekle', '/checkout.html')); }
  ...
  function demoSummary() {
    ...
    return { ok: true, user: { id: sessionUser.id || 'local', email: sessionUser.email || 'cankolsun@cosmoskin.com.tr', created_at: sessionUser.created_at || '2026-04-22T10:00:00+03:00', first_name: sessionUser.user_metadata?.first_name || 'Can', last_name: sessionUser.user_metadata?.last_name || 'Kölsün', full_name: sessionUser.user_metadata?.full_name || 'Can Kölsün', phone: sessionUser.user_metadata?.phone || '', skin_type: 'Karma Cilt', skin_concerns: ['Nem','Işıltı','Bariyer desteği'], routine_goal: 'Nem desteği', skin_profile_updated_at: '2026-06-27T12:00:00+03:00', communication: {} }, stats: { order_count: 0, active_order_count: 0, favorites_count: 0, addresses_count: 0, unread_notifications: 0, points_balance: 0 }, points: { balance: 0, ledger: [] }, coupons: [], orders: [], addresses: [], favorites: [], notifications: [] };
  }
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', function (e) {
      ...
      var addAddress = e.target.closest('#addAddressBtn'); if (addAddress) { window.location.href = '/checkout.html'; }
    ...
    document.addEventListener('submit', function (e) { if (e.target.matches('#passwordForm')) { e.preventDefault(); updatePassword(e.target).catch(handleError); } });
  ...
  async function init() {
    try {
      ...
      } else { state.session = { user: { email: 'cankolsun@cosmoskin.com.tr', user_metadata: { first_name: 'Can', last_name: 'Kölsün', full_name: 'Can Kölsün' } } }; }
      await loadSummary();
