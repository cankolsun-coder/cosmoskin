...
-- -----------------------------------------------------------------------------
-- COSMOSKIN FINAL USER ACCEPTANCE PATCH — coupons, bank-transfer reminder/cancel
-- Idempotent append-only migration. No fake payment keys, no destructive data reset.
-- -----------------------------------------------------------------------------
...
alter table if exists public.coupons add column if not exists title text;
alter table if exists public.coupons add column if not exists description text;
alter table if exists public.coupons add column if not exists discount_type text not null default 'percent';
alter table if exists public.coupons add column if not exists discount_value numeric(12,2) not null default 0;
alter table if exists public.coupons add column if not exists min_subtotal numeric(12,2) not null default 0;
alter table if exists public.coupons add column if not exists max_discount_amount numeric(12,2);
alter table if exists public.coupons add column if not exists usage_limit integer;
alter table if exists public.coupons add column if not exists per_customer_limit integer not null default 1;
alter table if exists public.coupons add column if not exists starts_at timestamptz;
alter table if exists public.coupons add column if not exists ends_at timestamptz;
alter table if exists public.coupons add column if not exists is_active boolean not null default true;
alter table if exists public.coupons add column if not exists scope_label text;
alter table if exists public.coupons add column if not exists created_at timestamptz not null default now();
...
begin
  update public.coupons
     set title = 'Yeni üyeye özel %10 hoş geldin avantajı',
         description = '1.000 TL ve üzeri ilk alışverişte geçerlidir.',
  ...
  if not exists (select 1 from public.coupons where upper(code) = 'WELCOME10') then
    insert into public.coupons (code, title, description, discount_type, discount_value, min_subtotal, per_customer_limit, is_active, scope_label)
    values ('WELCOME10', 'Yeni üyeye özel %10 hoş geldin avantajı', '1.000 TL ve üzeri ilk alışverişte geçerlidir.', 'percent', 10, 1000, 1, true, '1.000 TL ve üzeri ilk alışverişte');
  end if;

  update public.coupons set is_active = false, updated_at = now() where upper(code) = 'CLUB10';
end $$;
...
create table if not exists public.customer_coupons (
  ...
  discount_value numeric(12,2) default 10,
  min_subtotal numeric(12,2) default 1000,
  scope_label text,
  expires_at timestamptz,
  used_at timestamptz,
  order_id uuid,
...
alter table if exists public.customer_coupons add column if not exists user_id uuid;
alter table if exists public.customer_coupons add column if not exists customer_email text;
alter table if exists public.customer_coupons add column if not exists code text;
alter table if exists public.customer_coupons add column if not exists title text;
alter table if exists public.customer_coupons add column if not exists description text;
alter table if exists public.customer_coupons add column if not exists status text not null default 'available';
alter table if exists public.customer_coupons add column if not exists discount_type text default 'percent';
alter table if exists public.customer_coupons add column if not exists discount_value numeric(12,2) default 10;
alter table if exists public.customer_coupons add column if not exists min_subtotal numeric(12,2) default 1000;
alter table if exists public.customer_coupons add column if not exists scope_label text;
alter table if exists public.customer_coupons add column if not exists expires_at timestamptz;
alter table if exists public.customer_coupons add column if not exists used_at timestamptz;
alter table if exists public.customer_coupons add column if not exists order_id uuid;
alter table if exists public.customer_coupons add column if not exists created_at timestamptz not null default now();
alter table if exists public.customer_coupons add column if not exists updated_at timestamptz not null default now();
create index if not exists customer_coupons_user_code_idx on public.customer_coupons (user_id, upper(code));
create index if not exists customer_coupons_email_code_idx on public.customer_coupons (lower(customer_email), upper(code));
...
alter table if exists public.coupon_redemptions add column if not exists coupon_id uuid;
alter table if exists public.coupon_redemptions add column if not exists order_id uuid;
alter table if exists public.coupon_redemptions add column if not exists user_id uuid;
alter table if exists public.coupon_redemptions add column if not exists customer_email text;
alter table if exists public.coupon_redemptions add column if not exists code text;
alter table if exists public.coupon_redemptions add column if not exists discount_amount numeric(12,2) not null default 0;
alter table if exists public.coupon_redemptions add column if not exists status text not null default 'used';
alter table if exists public.coupon_redemptions add column if not exists metadata jsonb not null default '{}'::jsonb;
alter table if exists public.coupon_redemptions add column if not exists created_at timestamptz not null default now();
...
create index if not exists coupon_redemptions_customer_idx on public.coupon_redemptions (lower(customer_email), upper(code));

-- Email-event constraints: keep broad enough for manual bank-transfer operations.
do $$
declare r record;
begin
  if to_regclass('public.email_events') is not null then
    ...
    alter table public.email_events add constraint email_events_email_type_final_check check (email_type in (
      'order_created','payment_success','payment_confirmed_manual','bank_transfer_pending','bank_transfer_reminder','bank_transfer_not_received_cancelled','order_preparing','order_packed','shipment_created','shipment_updated','shipment_delivered','payment_failed','return_request_received','return_approved','return_rejected','refund_completed','review_request','restock_alert'
    ));
  end if;
end $$;

-- Order status events should accept new manual payment-not-received audit types.
do $$
declare r record;
begin
  if to_regclass('public.order_status_events') is not null then
    for r in
      ...
      join pg_namespace n on n.oid = t.relnamespace
      where n.nspname = 'public'
        and t.relname = 'order_status_events'
        and c.contype = 'c'
        and (pg_get_constraintdef(c.oid) ilike '%event_type%' or pg_get_constraintdef(c.oid) ilike '%status%')
    loop
      execute format('alter table public.order_status_events drop constraint if exists %I', r.conname);
    end loop;
    -- Existing production data may contain legacy/null event_type and status values.
    -- Keep the constraint future-safe but do not reject historical rows from older admin/order flows.
    alter table public.order_status_events add constraint order_status_events_status_final_check check (status is null or status in (
      'pending','confirmed','pending_payment','pending_bank_transfer','awaiting_transfer','payment_awaiting_transfer',
      'paid','payment_success','payment_confirmed','payment_confirmed_manual','payment_failed','failed','payment_authorized',
      'initiated','initialize_failed','processing','preparing','order_preparing','packed','order_packed','shipped','delivered',
      'shipment_created','shipment_updated','shipment_delivered','cancelled','bank_transfer_not_received',
      'stock_reserved','stock_released','updated','return_requested','return_approved','return_rejected','returned',
      'refunded','partially_refunded','refund_pending','refund_completed','refund_failed','invoice_pending','invoice_issued','invoice_cancelled',
      'review_required','order_processing_review_required','payment_duplicate_ignored'
    )) not valid;
    alter table public.order_status_events add constraint order_status_events_event_type_final_check check (event_type is null or event_type in (
...
-- RLS safety for account coupons where auth.uid() exists.
alter table if exists public.customer_coupons enable row level security;
do $$
begin
  if to_regclass('public.customer_coupons') is not null then
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='customer_coupons' and policyname='customer_coupons_select_own') then
      create policy customer_coupons_select_own on public.customer_coupons for select using (user_id = auth.uid());
    end if;
  end if;
end $$;
