...
for (const [file, text] of customerFacing) {
  if (/Checkout/i.test(text)) failures.push(`${file}: customer-facing Checkout copy remains`);
...
if (!/hasSuccessfulOrder/.test(accountJs) || !/WELCOME10/.test(accountJs) || !/welcomeEligible/.test(accountJs)) failures.push('WELCOME10 eligibility guard missing.');
...
if (!/birthday/.test(profileApi) || !/birth_date_locked/.test(profileApi) || !/birthday/.test(accountJs)) failures.push('Birth date profile flow missing.');
...
if (!/cs-overview-main-card--minimal/.test(accountJs) || !/cs-overview-grid--final-minimal/.test(css)) failures.push('Final overview minimal layout guard missing.');
if (!/focus-visible/.test(css) || !/outline:0/.test(css)) failures.push('Premium focus/blue outline guard missing.');
if (!/header\.account-header/.test(css) || !/letter-spacing:\.42em/.test(css)) failures.push('Account header parity guard missing.');
