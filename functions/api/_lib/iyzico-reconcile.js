import { insertRow, rpc, selectRows, updateRows } from './supabase.js';
import { sendOrderStatusEmail } from './order-email.js';
import { recordEmailEvent } from './email-events.js';
import { deriveCommerceSegments, mapSegmentsToLists, upsertBrevoContact } from './brevo.js';
import { iyzicoRequest } from './iyzico.js';
import { recordCrmEvent } from './crm-events.js';
import { ensureShipmentShell, finalizeCommerceAfterPayment } from './commerce-finalization.js';

/**
 * Shared iyzico "checkoutform/auth/ecom/detail" verify-and-finalize path.
 * Used by both the synchronous /api/iyzico-callback POST handler and the
 * /api/cron/reconcile-pending-payments safety-net job — both must run the
 * exact same security-sensitive verification (assertPaymentBinding) and the
 * exact same idempotent finalize sequence, so this lives in one place.
 */
...
async function recordStatusEvent(context, orderId, status, message, metadata = {}) {
  ...
  try {
    await insertRow(context, 'order_status_events', {
      ...
      source: 'payment',
      created_by: metadata.source === 'reconcile_cron' ? 'payment_reconcile_cron' : 'payment_callback',
      message,
      note: message,
      metadata
    });
...
export async function reconcileIyzicoPaymentByToken(context, token, { source = 'callback' } = {}) {
  let orderId = '';
  let paymentWasSuccessful = false;
  try {
    if (!context.env.IYZICO_API_KEY || !context.env.IYZICO_SECRET_KEY) {
      return { ok: false, orderId: null, success: false, reason: 'iyzico_not_configured' };
    }
    if (!token) return { ok: false, orderId: null, success: false, reason: 'token_missing' };

    const paymentRow = await findPaymentByToken(context, token).catch(() => null);
    if (!paymentRow?.order_id) return { ok: false, orderId: null, success: false, reason: 'payment_not_found' };
    ...
    const metadata = { ...callbackMetadata(retrieve), source };
    ...
    if (success) {
      ...
      try {
        ...
        console.error('iyzico paid inventory processing failed:', { orderId, source, code: error?.code || null, message: inventoryProcessingError });
      }
      ...
      if (paymentVerifiedButProcessingFailed) {
        ...
        return { ok: true, orderId, success: true, reviewRequired: true };
      }

      if (processing?.claimed === false) {
      ...
      if (confirmationEmailEligible) {
        ...
        await recordCrmEvent(context, { event_type: 'purchase_completed', order_id: orderId, metadata: { source, payment_id: providerPaymentId || null } });
      }
      return { ok: true, orderId, success: true, reviewRequired: false, duplicate: processing?.claimed === false };
    }
    ...
    return { ok: true, orderId, success: false, duplicate: failureProcessing?.claimed === false };
  } catch (error) {
    console.error('iyzico reconcile failed:', { orderId: orderId || null, source, code: error?.code || null, message: String(error?.message || 'unknown').slice(0, 300) });
    if (orderId) {
      await recordStatusEvent(context, orderId, paymentWasSuccessful ? 'inventory_reconciliation_required' : 'payment_callback_error', paymentWasSuccessful ? 'Ödeme onaylandı ancak callback tam olarak işlenemedi; manuel kontrol gerekli.' : 'Ödeme callback doğrulanamadı veya işlenemedi.', { error_code: error?.code || null, error: String(error?.message || 'unknown').slice(0, 300), source });
    }
    return { ok: false, orderId: orderId || null, success: paymentWasSuccessful, error: String(error?.message || 'unknown').slice(0, 300) };
  }
}
