...
export async function onRequestGet(context) { return handleApi(async () => { const { supabase, user } = await requireAccount(context); const url = new URL(context.request.url); return ok(await getRecommendations(supabase, user, url.searchParams.get('limit') || 8)); }); }
