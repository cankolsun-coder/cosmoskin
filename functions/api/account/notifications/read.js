...
export async function onRequestPost(context) {
  return handleApi(async () => {
    ...
    const data = payload.readAll === true ? await markAllRead(supabase, user) : await markRead(supabase, user, Array.isArray(payload.notificationIds) ? payload.notificationIds : [payload.notificationId || payload.id].filter(Boolean));
