...
export async function onRequestPost(context) { return handleApi(async () => { const { supabase, user } = await requireAccount(context); const payload = await parseJson(context.request); return ok(await completeRoutine(supabase, user, payload)); }); }
