...
export async function onRequestGet(context) { return handleApi(async () => { const { supabase, user } = await requireAccount(context); const routine = await getTodayRoutine(supabase, user); const routineStreak = await getRoutineStreak(supabase, user); return ok({ routine, todayRoutine: routine, routineStreak }); }); }
