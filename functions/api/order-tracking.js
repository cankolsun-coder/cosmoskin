...
function publicOrder(order = {}, items = [], shipments = []) {
  return {
    id: order.id,
...
export async function onRequestPost(context) {
  try {
    ...
    const orderNumber = cleanOrderNumber(body.order_number || body.orderNumber);
    ...
    if (!order) return json({ ok: false, error: 'Bu sipariş numarası ve e-posta ile eşleşen kayıt bulunamadı.' }, { status: 404 });
    ...
    return json({ ok: true, order: publicOrder(order, items, shipments) });
  } catch (error) {
