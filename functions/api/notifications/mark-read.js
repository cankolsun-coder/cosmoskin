...
export async function onRequestPost(context) {
  return handleApi(async () => {
    const { supabase, user } = await requireAccount(context);
    const payload = await parseJson(context.request);
    const ids = Array.isArray(payload.notificationIds) ? payload.notificationIds : [payload.notificationId || payload.notification_id || payload.activity_id || payload.id].filter(Boolean);
    const data = await markRead(supabase, user, ids);
